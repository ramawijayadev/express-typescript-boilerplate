...
*   **Dummy Hash Salt:** The dummy hash string in `auth.service.ts` is hardcoded. While low risk (as it's only for timing mitigation), rotating it requires a code change. *Recommendation: Move to env var.*
*   **Sanitization Regex:** The `sanitizeObject` function uses a basic regex `/[${}]/g` to strip characters. This is a "blunt instrument" approach. It works for preventing Mongo-style injection but might inadvertently strip valid user input requiring those characters. *Recommendation: Ensure this doesn't break valid business logic (e.g. if users need to type `$` in a bio).*
