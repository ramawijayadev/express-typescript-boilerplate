...
*   **Middleware Registration:** In `app.ts`, `swaggerUi` is mounted inside the `NODE_ENV !== "production"` block. This is good, but `app.use("/", swaggerUi.serve)` is called *after* `app.get(...)`. It works, but usually `serve` is static middleware and could be defined once. (Minor).
